...
class AddView(generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        if 'save' in self.request.POST:
            ...
            messages.success(self.request, "追加しました")
            return redirect('bookmgr:book_add')

        # 保存して編集を続けるボタンのとき
        elif 'save_and_edit' in self.request.POST:
            messages.success(self.request, "追加しました")
            return redirect('bookmgr:book_update', pk=book.pk)
...
class ImpressionAddView(generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        book.save()
        messages.success(self.request, "追加しました")
        return redirect('bookmgr:impression_list', pk=book_pk)
...
class ImpressionUpdateView(generic.UpdateView):
    ...
    template_name = 'impression_add.html'
    form_class = ImpressionCreateForm
    success_url = reverse_lazy('bookmgr:impression_list')
...
def impression_edit(request, book_id, impression_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():  # フォームのバリデーション
            ...
            impression.save()
            return redirect('bookmgr:impression_list', book_id)
...
def impression_delete(request, book_id, impression_id):
    ...
    book.impressionCount = b['impre_count']
    book.save()
    return redirect('bookmgr:impression_list', pk=book_id)
...
#     success_url = reverse('bookmgr:impression_list')
#
#     def form_valid(self, form):
#         ''' バリデーションを通った時 '''
#         messages.success(self.request, "削除しました")
